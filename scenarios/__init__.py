...
class FhirUser(HttpUser):
    ...
    def get_random_patient_row(self):
        ...
        res = fixtures_manager.cursor().execute(
            f"SELECT id, given, family, phone, gender, birth_date, address, city, state "
    ...
    def get_random_observation_row(self):
        ...
        res = fixtures_manager.cursor().execute(
            f"SELECT id, issued, subject, status, code, encounter, category, value_quantity, value_concept "
    ...
    def get_random_organization_row(self):
        ...
        res = fixtures_manager.cursor().execute(
            f"SELECT id, name, active, type from organization where id = {_id}"
        )
        row = res.fetchone()
    ...
    def get_random_practitioner_row(self):
        ...
        res = fixtures_manager.cursor().execute(
            f"SELECT id, identifier, active, name from practitioner where id = {_id}"
        )
        row = res.fetchone()
    ...
    def get_random_encounter_row(self):
        ...
        res = fixtures_manager.cursor().execute(
            f"SELECT id, status, subject, period_start, period_end  from encounter where id = {_id}"
        )
        row = res.fetchone()
    ...
    def get_random_condition_row(self):
        ...
        res = fixtures_manager.cursor().execute(
            f"SELECT id, subject, encounter, recorder, asserter from condition where id = {_id}"
        )
        row = res.fetchone()
    ...
    def get_random_composition_row(self):
        ...
        res = fixtures_manager.cursor().execute(
            f"SELECT id, subject, status, encounter, author, section from composition where id = {_id}"
        )
        row = res.fetchone()
